...
window.addEventListener("DOMContentLoaded", () => {
  ...
  configButton.addEventListener("submit", function (event) {
    ...
    // 插入运行状态信息
    let status = document.getElementById("msg-box").children[0];
    ...
    python.on("close", (code) => {
      ...
      status.innerHTML="Successful <br> Authentication"
    });
  });
  // 监视reset事件
  configButton.addEventListener("reset", function (event) {
    ...
    let msg_box = document.getElementById("msg-box").children[0];
    msg_box.innerHTML = "";
