...
class Protocol_harmony_c_action:
    ...
    def compute_M1(self, A, B, K):
        ...
        :return:
        """
        return self.HashFunc.Hash(
            self.curve.to_bytes(A) + self.curve.to_bytes(B) + str(K).encode("utf-8")
        )
...
def Protocol_harmony_c(
    HOST: str, port: int, passwd: str, curve_name: str = "Ed25519", debug: bool = False
) -> list:
    """
    IoT设备参与协议的运行
    ...
    conn.send((QR + salt).encode("UTF-8"))
    ...
    B = conn.recv(1024)
    if debug:
        print(f"[key] send parameter <B>\n[val] 0x{B.hex()}")
    u = conn.recv(1024)
    if debug:
        print(f"[key] send parameter <u>:\n[val] {u}")
    ...
    conn.send(str(M1).encode("utf-8"))
    if debug:
        print(f"[key] send parameter <M1>:\n[val] {M1}")
    # Step Seven IoT设备验证M2
    M2 = conn.recv(1024).decode("utf-8")
    M2 = int(M2)
    if IoT.verfy_M2(A, M1, K, M2) == 0:
        return False
    if debug:
        print(f"[sta] verify parameter <M2> successful")
    # Step Eight IoT设备加密传输身份标识公钥c1
    c1 = IoT.encrypt(K, IoT.public_key)
    conn.send(c1)
    if debug:
        print(f"[key] send parameter <c1>:\n[val] 0x{c1.hex()}")
    ...
    conn.send(IoT.curve.to_bytes(Z))
    cipher_iots = conn.recv(1024)
...
class Protocol_harmony_s_action:
    ...
    def compute_M2(self, A, M1, K):
        ...
        """
        return self.HashFunc.Hash(
            self.curve.to_bytes(A) + str(M1).encode("UTF-8") + str(K).encode("utf-8")
        )
...
def Protocol_harmony_s(
    HOST: str, port: int, curve_name: str = "Ed25519", debug: bool = False
) -> list:
    """
    IoT主控设备参与协议的运行
    ...
    conn.send(c2)
    if debug:
        print(f"[key] send parameter <encrypted identity public key>\n[val] {c2}")
    ...
    if debug:
        print(
            f"[key] send parameter <encrypted signature>\n[val] 0x{cipher_iots.hex()}"
        )
